...
app.post('/api/usuarios', requireRole('admin'), async (req, res) => {
  try {
    ...
    if (password && password.length < 6) errors.push('La contraseña debe tener al menos 6 caracteres');
     if (!role || !['admin', 'user', 'administrador'].includes(role)) {
      errors.push('Rol inválido (use admin, user o administrador)');
    }
...
app.put('/api/usuarios/:id', requireRole('admin'), async (req, res) => {
  try {
    ...
    if (role != null && !['admin', 'user', 'administrador'].includes(role)) {
      errors.push('Rol inválido (use admin, user o administrador)');
    }
...
app.get('/usuarios', requireAuth, async (req, res) => {
  ...
  // Si es admin, cargar y renderizar la lista de usuarios
  const usuarios = await User.findAll({
    order: [
      // opcional: poner primero al usuario actual, luego por username
      [sequelize.literal(`CASE WHEN id = ${req.user.id} THEN 0 ELSE 1 END`), 'ASC'],
      ['username', 'ASC']
    ]
  });

  res.render('usuarios', { usuarios: usuarios.map(userToDTO) });
});

 app.get('/usuarios', requireRole('admin'), async (req, res) => {
  const currentId = req.user ? req.user.id : 0;
  const usuarios = await User.findAll({
    order: [
      // Primero el usuario logueado, luego los que tienen role='user', luego el resto; dentro de cada grupo orden por username.
      [sequelize.literal(`CASE WHEN id = ${currentId} THEN 0 WHEN role = 'user' THEN 1 ELSE 2 END`), 'ASC'],
      ['username', 'ASC']
    ]
  });
  res.render('usuarios', {
